...
/*
    2D thread tile
    one thread compute more elements
*/

template <const int BM, const int BN, const int BK, const int TM, const int TN>
__global__ void sgemm2(int M, int N, int K, float *A, float *B, float *C)
{
...
#pragma unroll
                for (int c = 0; c < TN; c++)
                {
                    tmp[r][c] += s_A[(ty + r) * BK + i] * s_B[i * BN + (tx + c)];
                }
            }
        }
        __syncthreads();
...
int main()
{
    ...
    sgemm2<BLOCK_M, BLOCK_N, BLOCK_K, THREAD_M, THREAD_N><<<grid, block>>>(M, N, K, d_A, d_B, d_C);
