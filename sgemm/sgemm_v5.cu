...
/*
    prefetch data by double buffers
*/
    ...
    do {
        ...
        for (int bk = 0; bk < BK - 1; bk++) {
            ...
            for (int m = 0; m < TM; m += 4) {
                FETCH_FLOAT4(a_frag[(bk + 1) % 2][m]) = FETCH_FLOAT4(s_A[load_index][OFFSET(bk + 1, ty + m, BM)]);
            }
            #pragma unroll
            for (int n = 0; n < TN; n += 4) {
                FETCH_FLOAT4(b_frag[(bk + 1) % 2][n]) = FETCH_FLOAT4(s_B[load_index][OFFSET(bk + 1, tx + n, BN)]);
            }
            // 计算tile
            #pragma unroll
            for (int r = 0; r < TM; r++) {
                #pragma unroll
                for (int c = 0; c < TN; c++) {
                    tmp[r][c] += a_frag[bk % 2][r] * b_frag[bk % 2][c];
                }
            }
        }
